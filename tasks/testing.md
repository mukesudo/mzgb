...
- [ ] T-01 [stream] Pipe a 500 MB file through `mzgb` and verify memory stays below 50 MB
  - Setup: generate a large file with `yes "INFO test" | head -5000000 > /tmp/big.log`
  - Command: `mzgb /tmp/big.log | wc -l`
...
- [ ] T-02 [filter] Level filter returns only matching lines
  ...
  - Command: `mzgb --level ERROR test.log | grep -v ERROR`
